...
Test Setup          Accsses    ${locale}    PhoneWatches
...
2-Validate content - FeaturesDesigned
    ...
    [Tags]    content
    Search element    xpath=//h1[contains(@id,'FeaturesTitle_desktop')]/../../..
    Log    Dicionário: ${DIC_DESIGNED}
    Sleep    2s
    #Title Giving Them Freedom...
    Validate content, JSON    id=FeaturesTitle_desktop    ${DIC_DESIGNED["title"]}
    #Paragraph Moochies is on a mission to keep...
    Validate content, JSON    id=greatForKidsTitle    ${DIC_DESIGNED.card01.title}
    #Paragraph With GPS tracking so you’ll know where they are....
    Validate content, JSON    id=greatForKidsParagraph    ${DIC_DESIGNED.card01.paragraph}
    #Paragraph Every child deserves a childhood of fun
    Validate content, JSON    id=MonthWarrantyTitle  ${DIC_DESIGNED.card02.title}
    #Paragraph Every child deserves a childhood of fun
    Validate content, JSON    id=MonthWarrantyParagraph  ${DIC_DESIGNED.card02.paragraph}
...
5-Validate content - MoreAbout
    [Documentation]    Validação do Texto presente no elemento ContactHeader - Content ContactUs Page
    [Tags]    content
    Search element    xpath=//h1[contains(@id,'moreAboutTitle_desktop')]/..
    Log    Dicionário: ${DIC_MOREABOUT}
    Sleep    2s
    #Title Giving Them Freedom...
    Validate content, JSON    id=moreAboutTitle_desktop    ${DIC_MOREABOUT["title"]}
    #Paragraph Moochies is on a mission to keep...
